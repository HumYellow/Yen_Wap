...
.designFilter .designFilterHead .designFilterMod{width:50%;float:left;text-align:center;padding:20px 0;color:#8e8e8e;}
...
#designList .ajaxOvere{font-size:14px;padding:0 30px;}
</style>
<template>
	<div class="design">
		<div class="designFilter">
			<div class="designFilterHead clear" v-if="!centerType">
				<div class="designHead">Design</div>
...
export default {
	...
	methods: {
				...
				for(var i = 0;i<this.designFilterList.length;i++){//替换切换赋值
					for(var a = 0;a<this.designFilterList[i].subTypeList.length;a++){
						...
						if(arr.indexOf(this.designFilterList[i].subTypeList[a].designTypeId) > -1){
							this.designFilterList[i].name = this.designFilterList[i].subTypeList[a].name
						}
					}
				}
				this.getDataing = false
		...
		isBottom(){
			...
			if(document.body.scrollHeight - scrollTop <= document.body.clientHeight+30){
				if(!this.lastPage){
					this.ajaxNew = false
					this.getData()
				}
		...
		getHeight(){
			...
			setTimeout(()=>{
				...
				if(this.getHeightTime > this.getHeightMaxTime){
					this.$layer.alert("Your network may have problems.please try again later.")
				}
